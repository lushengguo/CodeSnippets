...
private:
  void
  do_read(std::function<bool(size_t)> payload_length_consumer,
          std::function<bool(const std::span<uint8_t> &)> payload_consumer) {
    boost::asio::async_read(
        socket_, boost::asio::buffer(&payload_length_, sizeof(payload_length_)),
        [this, payload_length_consumer, payload_consumer](
          ...
          boost::asio::async_read(
              ...
              [this, payload_length_consumer, payload_consumer](
                ...
                if (not payload_consumer or
                    not payload_consumer(
...
setup_connection(const std::string &test_name = "") {
...
// 辅助函数：等待连接建立
bool wait_for_connection(std::future<bool> &future,
                         const std::string &test_name) {
...
// 辅助函数：创建基本的读取验证器
auto create_message_validator(const std::string &expected_message,
  ...
  return std::make_pair(
      ...
      [&read_complete, &expected_message,
       logger](const std::span<uint8_t> &data) {
...
// 辅助函数：创建批量数据读取验证器
auto create_bulk_validator(size_t message_length, size_t thread_count,
                           size_t each_thread_write_count,
  ...
  return std::make_pair(
      ...
      [&read_count, &read_complete, message_length, thread_count,
       each_thread_write_count, logger](const std::span<uint8_t> &data) {
        if (logger)
          logger->debug("Processing data chunk of size: {}", data.size());

        bool all_same = std::all_of(
            data.begin(), data.end(),
            [first = data[0]](uint8_t value) { return value == first; });
        ...
        if (logger)
          logger->debug("Total bytes read: {}", current_count + data.size());

        if (current_count + data.size() >=
...
// 辅助函数：同步写入
void sync_write(const std::shared_ptr<AsioServer> &server,
...
// 辅助函数：异步写入
void async_write(const std::shared_ptr<AsioServer> &server,
                 const std::vector<uint8_t> &payload,
                 const std::string &test_name) {
  auto logger = spdlog::get(test_name);
  if (logger)
...
// 辅助函数：协程写入
void coroutine_write(const std::shared_ptr<IoContextRunner> &runner,
                     const std::shared_ptr<AsioServer> &server,
                     const std::vector<uint8_t> &payload,
                     std::promise<std::size_t> &write_size_promise,
                     const std::string &test_name) {
  auto logger = spdlog::get(test_name);
  if (logger)
    logger->debug("Starting coroutine write with payload size: {}",
                  payload.size());
  ...
  boost::asio::co_spawn(
      ...
      [&, logger]() -> boost::asio::awaitable<void> {
        ...
        if (logger)
          logger->debug("Coroutine write completed, bytes written: {}",
                        bytes_written);
...
void multi_thread_write(const std::shared_ptr<IoContextRunner> &runner,
                        const std::shared_ptr<AsioServer> &server,
                        size_t message_length, size_t thread_count,
                        size_t each_thread_write_count, std::atomic_bool &stop,
                        WriteFunc write_func, const std::string &test_name) {
  auto logger = spdlog::get(test_name);
  ...
  for (size_t i = 0; i < thread_count; ++i) {
    threads[i].join();
    if (logger)
      logger->debug("Thread {} joined", i);
  }
}

TEST(asio_practice, TestConnect) {
  std::string test_name = "TestConnect";
...
TEST(asio_practice, TestSimpleWrite) {
  std::string test_name = "TestSimpleWrite";
  auto [runner, server, client, future] = setup_connection(test_name);
  ...
  auto [length_validator, payload_validator] =
      create_message_validator(message, read_complete, test_name);
...
TEST(asio_practice, TestSimpleAsyncWrite) {
  std::string test_name = "TestSimpleAsyncWrite";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
...
TEST(asio_practice, TestSimpleCoroutineWrite) {
  std::string test_name = "TestSimpleCoroutineWrite";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
...
TEST(asio_practice, TestAsyncWriteBulkOnce) {
  std::string test_name = "TestAsyncWriteBulkOnce";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
...
TEST(asio_practice, TestAsyncWriteMultithreadWithSingleThreadIoContext) {
  std::string test_name = "TestAsyncWriteMultithreadWithSingleThreadIoContext";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
  ...
  auto [length_validator, payload_validator] = create_bulk_validator(
      message_length, thread_count, each_thread_write_count, read_count,
      read_complete, test_name);

  client->read(length_validator, payload_validator);

  multi_thread_write(
      runner, server, message_length, thread_count, each_thread_write_count,
      stop,
...
TEST(asio_practice, TestAsyncWriteMultithreadWithMultiThreadIoContext) {
  std::string test_name = "TestAsyncWriteMultithreadWithMultiThreadIoContext";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
...
TEST(asio_practice, TestCoroutineWriteMultithreadWithSingleThreadIoContext) {
  std::string test_name =
      "TestCoroutineWriteMultithreadWithSingleThreadIoContext";
  auto [runner, server, client, future] = setup_connection(test_name);
  ASSERT_TRUE(wait_for_connection(future, test_name));
